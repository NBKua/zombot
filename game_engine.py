...
from game_actors_and_handlers.move_items import MoveItem
from game_actors_and_handlers.BuildingTrack import BuildingTrack
from game_actors_and_handlers.exchange import Exchange_Collection
from game_actors_and_handlers.create_shovel import CreateShovelExtra
from game_actors_and_handlers.exchange_all import GameBrains1
from game_actors_and_handlers.deletingobjects import DeletingObjects
...
from game_actors_and_handlers.exchange_izumrud import ExhangeCollEmerald
from game_actors_and_handlers.storage_put import StoragePut
...
class GameState():
    ...
    def __init__(self, start_response, item_reader):
        ...
        if len(self.__game_state.buyedBrains)<>0:
          ...
          logger.info("")
	  zb=str(self.__game_state.cashMoney)
	  logger.info(u'ЗБ у игрока'+(str(self.__game_state.cashMoney)))

	  for itemid in self.__game_state.storageItems:
	      if itemid.item == "@DUBLON":
	          logger.info(u'Дублонов у игрока: '+(str(itemid.count)))
	  for itemid in self.__game_state.storageItems:
	      if itemid.item == "@SHOVEL_EXTRA":
	          logger.info(u'Лопат у игрока: '+(str(itemid.count)))
...
class Game():
    ...
    CLIENT_VERSION = long(1406282417)
...
#                GetMissionsBot(self.__itemReader, self.__game_state_, self.__game_events_sender, self._get_timer(),{}).perform_action()
    ...
    def create_all_actors(self):
        ...
        actor_classes = [
	    StoragePut,          #выставление сосклада барахло
	    #ExhangeCollEmerald,     # создание изумрудной
	    #BowReceiverBot,        #сбор бантов
	    #Emeraldic,             #создание луксорской
            #GameBrains1,           #создание коллекций и мозгов
            #GameBrains,          # производство мозгов (останкинобкорабль)
            ...
            #Exchange_Collection,  # обмен коллекции(любая)
	    #MoveItem,              #воровать вещи с острова(не актуально)
	    #TreePlant,           #Посадка деревьев
            #BuildingTrack,        # покупка настилов(не актуально)
            #CreateShovelExtra,    # Создание лопат в Глаз-алмаз
            #DeletingObjects,       # Удаление объектов
            #BoltGift,             #передача другу чего либо
            #ExchangeCollections,
